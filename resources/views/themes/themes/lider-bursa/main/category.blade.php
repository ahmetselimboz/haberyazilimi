                    ...
                    <div id="headlineCarousel" class="carousel" data-bs-ride="carousel" data-ride="carousel">
                        <div class="carousel-inner">
                                    ...
                                    <div class="carousel-item @if ($key == 0) active @endif">
                                        <div class="headline-item">
                                            <div class="headline-image">
                                                <img width="100%" height="510" src="{{ imageCheck($post->images) }}"
                                                    class="lazy mobilserbest">
                                            </div>
                            ...
                            <button class="carousel-control-prev d-flex" type="button"
                                    >
                                <span class="carousel-control-prev-icon" aria-hidden="true" data-bs-target="#headlineCarousel" data-bs-slide="prev"></span>
                                <span class="visually-hidden">Önceki</span>
                            </button>

                            <button class="carousel-control-next d-flex" type="button"
                                   >
                                <span class="carousel-control-next-icon" aria-hidden="true"  data-bs-target="#headlineCarousel" data-bs-slide="next"></span>
                        ...
                        @else
                            <div class="ad-block">
                                <a href="{{ adsCheck($ads6->id)->url }}" title="Reklam {{ $ads6->id }}"
                                    class="externallink">
                                    <img src="{{ asset(adsCheck($ads6->id)->images) }}" alt="Reklam {{ $ads6->id }}"
                                        class="img-fluid lazy" data-type="{{ adsCheck($ads6->id)->type }}"
                ...
                @else
                    <div class="ad-block">
                        <a href="{{ adsCheck($ads7->id)->url }}" title="Reklam {{ $ads7->id }}" class="externallink">
                            <img src="{{ asset(adsCheck($ads7->id)->images) }}" alt="Reklam {{ $ads7->id }}"
                                class="img-fluid lazy" data-type="{{ adsCheck($ads7->id)->type }}"
                                height="{{ adsCheck($ads7->id)->height }}" width="{{ adsCheck($ads7->id)->width }}">
                        </a>
                    </div>
                @endif
            </div>
                ...
                @else
                    <div class="spot spotduz spotduz-{{ $counter }}">
                        <a href="{{ route('post', ['categoryslug' => $post->category->slug, 'slug' => $post->slug, 'id' => $post->id]) }}"
                            title="{{ html_entity_decode($post->title) }}">
                            <img src="{{ route('resizeImage', ['i_url' => imageCheck($post->images), 'w' => 380, 'h' => 253]) }}"
                                alt="{{ $post['title'] }}" alt="{{ html_entity_decode($post->title) }}" />
                            <p><span>{{ html_entity_decode($post->title) }}</span></p>
                        </a>
                    </div>
                @endif
