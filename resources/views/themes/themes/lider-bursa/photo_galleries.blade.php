            ...
            @foreach($photo_galleries as $key => $photo_gallery)
                <div class="row">
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        @if(adsCheck($ads11->id) && adsCheck($ads11->id)->publish==0)
                            ...
                            @else
                                <div class="ad-block">
                                    <a href="{{ adsCheck($ads11->id)->url }}" title="Reklam {{$ads11->id}}" class="externallink">
                                        <img src="{{ asset(adsCheck($ads11->id)->images) }}" alt="Reklam {{$ads11->id}}" class="img-fluid lazy" data-type="{{ adsCheck($ads11->id)->type }}" height="{{ adsCheck($ads11->id)->height }}" width="{{ adsCheck($ads11->id)->width }}">
                                    </a>
                                </div>
                            @endif
                        @endif
...
@if(adsCheck($ads12->id) && adsCheck($ads12->id)->publish==0)
        ...
        @else
            <div class="ad-block">
                <a href="{{ adsCheck($ads12->id)->url }}" title="Reklam {{$ads12->id}}" class="externallink">
                    <img src="{{ asset(adsCheck($ads12->id)->images) }}" alt="Reklam {{$ads12->id}}" class="img-fluid lazy" data-type="{{ adsCheck($ads12->id)->type }}" height="{{ adsCheck($ads12->id)->height }}" width="{{ adsCheck($ads12->id)->width }}">
                </a>
            </div>
        @endif
    </div>
