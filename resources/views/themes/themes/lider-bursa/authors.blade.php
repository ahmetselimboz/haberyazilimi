        ...
        <div class="row">
                            ...
                            <div class="author-block overflow-hidden rounded-1 border-0">
                                <div class="author-block-article">
                                    <h5 class="text-truncate-line-3">
                                        <a href="{{ route('article',['slug'=>$author->latestArticle->slug,'id'=>$author->latestArticle->id]) }}" class="externallink text-dark" title="{{ $author->latestArticle->title }}">{{ $author->latestArticle->title }}</a></h5>
                                    <p class="text-truncate-line-2">
                                        <a href="{{ route('article',['slug'=>$author->latestArticle->slug,'id'=>$author->latestArticle->id]) }}"
                                            class="externallink text-dark" title="{{ $author->latestArticle->title }}">
                                        {!! Str::limit(strip_tags($author->latestArticle->detail), 150, '...') !!}
                                    </a>
                                    </p>
                                </div>
                                <div class="author-block-footer d-flex">
                                    <a href="#" title="{{ $author->name }} tüm yazıları" class="externallink">
                                        <img src="
                                        {{ route('resizeImage', ["i_url" => $author->avatar, "w" => 100, "h" => 100]) }}
                                            " class="img-thumbnail lazy" alt="{{ $author->name }}">
                                        <div>
                                            <h6 class="mb-0 text-truncate">{{ $author->name }}</h6>
                                            <small  class="text-opacity-50 text-truncate">{{ date('d/m/Y', strtotime($author->latestArticle->created_at)) }}</small>
                                            <a href="{{ route('article',['slug'=>$author->latestArticle->slug,'id'=>$author->latestArticle->id]) }}" class="all-article-link externallink" title="{{ $author->name }} tüm yazıları"><i class="curret-right"></i> Tüm Yazıları</a>
                                        </div>
                                    </a>
                                </div>
                            </div>
            ...
            <div class="col-12 col-md-4"><!-- (Öne Çıkan Haberler) -->
                ...
                @if(adsCheck($ads17->id) && adsCheck($ads17->id)->publish==0)
                    ...
                    @else
                        <div class="ad-block">
                            <a href="{{ adsCheck($ads17->id)->url }}" title="Reklam {{$ads17->id}}" class="externallink">
                                <img src="{{ asset(adsCheck($ads17->id)->images) }}" alt="Reklam {{$ads17->id}}" class="img-fluid lazy" data-type="{{ adsCheck($ads17->id)->type }}" height="{{ adsCheck($ads17->id)->height }}" width="{{ adsCheck($ads17->id)->width }}">
                            </a>
                        </div>
                    @endif
                @endif
