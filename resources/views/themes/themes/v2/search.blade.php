            ...
            @foreach ($posts as $post)
                <div class="col-6 col-lg-3 mb-4"><!-- (Haber Kartı) -->
                    <div class="card overflow-hidden rounded-1 border-0 shadow-sm">
                        <a href="{{ route('post', ['categoryslug' => $post->category->title, 'slug' => $post->slug, 'id' => $post->id]) }}"
                            title="{{ $post->title }}" class="externallink">
                            <img src="{{ imageCheck($post->images) }}"
                                class="card-img-top rounded-0 lazy post-image-height img-fluid" alt="{{ $post->title }}">
                        </a>
                        <div class="card-body">
                            <a href="{{ route('post', ['categoryslug' => $post->category->title, 'slug' => $post->slug, 'id' => $post->id]) }}"
                                title="{{ $post->title }}" class="externallink">
