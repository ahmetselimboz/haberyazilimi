                            ...
                            @else
                                <div class="ad-block">
                                    <a href="{{ adsCheck($ads14->id)->url }}" title="Reklam {{$ads14->id}}" class="externallink">
                                        <img src="{{ asset('uploads/'.adsCheck($ads14->id)->images) }}" alt="Reklam {{$ads14->id}}" class="img-fluid lazy" data-type="{{ adsCheck($ads14->id)->type }}" height="{{ adsCheck($ads14->id)->height }}" width="{{ adsCheck($ads14->id)->width }}">
                                    </a>
                                </div>
                            @endif
                        @endif
                        ...
                        <div class="col-6 col-md-4 col-lg-2 mb-4"><!-- (Galeri Kartı) -->
                            <div class="card overflow-hidden rounded-1 border-0 shadow-sm">
                                <a href="@if($video_gallery->category!=null) {{ route('video_gallery', ['categoryslug'=>$video_gallery->category->slug,'slug'=>$video_gallery->slug,'id'=>$video_gallery->id]) }} @endif" class="externallink" title="{{ html_entity_decode($video_gallery->title) }}">
                                    <img src="
                                    {{ route('resizeImage', ["i_url" => imageCheck($video_gallery->images), "w" => 179, "h" => 119]) }}
