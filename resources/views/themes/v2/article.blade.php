...
@section('meta')
    ...
    <meta property="og:image" content="@if($article->author!=null) {{ $article->author->avatar }} @endif " />
    ...
    <meta name="twitter:image" content="@if($article->author!=null) {{ $article->author->avatar }} @endif" />
            ...
            @else
                <div class="ad-block">
                    <a href="{{ adsCheck($ads18->id)->url }}" title="Reklam {{$ads18->id}}" class="externallink">
                        <img src="{{ asset('uploads/'.adsCheck($ads18->id)->images) }}" alt="Reklam {{$ads18->id}}" class="img-fluid lazy" data-type="{{ adsCheck($ads18->id)->type }}" height="{{ adsCheck($ads18->id)->height }}" width="{{ adsCheck($ads18->id)->width }}">
                    </a>
                </div>
            @endif
        </div>
        ...
        <div class="row">
                    ...
                    <div class="col-12 mb-4">
                        <div class="author-detail-card overflow-hidden rounded-1 border-0">
                            <div class="author-detail-article">
                                <h5 class="">{{ html_entity_decode($article->title) }}</h5>
                            </div>
                            <div class="author-detail-footer d-flex">
                                <a href="#" title="yazar link">
                                    <img src="@if($article->author!=null)
                                    {{ route('resizeImage', ["i_url" => $article->author->avatar, "w" => 100, "h" => 100]) }}
                                    ...
                                    <div class="w-100 d-flex justify-content-between">
                                        <div>
                                            ...
                                            <small  class="text-white text-opacity-50 text-truncate">{{ date('d.m.Y - H:i', strtotime($article->created_at)) }}</small>
                                        </div>
                                        <div class="author-social mt-1 pt-1 d-none d-md-block">
                                            <div class="d-flex justify-content-end flex-wrap flex-lg-nowrap">
                        ...
                        <div class="author-detail-footer w-100 d-flex d-md-none px-2 mt-2 rounded-1">
                            ...
                            @else
                                <div class="ad-block">
                                    <a href="{{ adsCheck($ads19->id)->url }}" title="Reklam {{$ads19->id}}" class="externallink">
                                        <img src="{{ asset('uploads/'.adsCheck($ads19->id)->images) }}" alt="Reklam {{$ads19->id}}" class="img-fluid lazy" data-type="{{ adsCheck($ads19->id)->type }}" height="{{ adsCheck($ads19->id)->height }}" width="{{ adsCheck($ads19->id)->width }}">
                                    </a>
                                </div>
                            @endif
                        </div>
            ...
            <div class="col-12 col-lg-4">
                <div class="row">
                    ...
                    <!--Diğer Yazıları-->
                    <div class="col-12 mb-4">
                        <div class="mostly-block overflow-hidden rounded-1 shadow-sm bg-white">
                            <h2 class="mostly-block-headline mb-4 text-uppercase">Diğer Yazıları</h2>

                            <div class="container-fluid mb-3 pb-1">
                                <div class="row" style="height:600px;overflow-x:hidden;owerflow-y:auto;">
                    ...
                    @else
                        <div class="ad-block">
                            <a href="{{ adsCheck($ads20->id)->url }}" title="Reklam {{$ads20->id}}" class="externallink">
                                <img src="{{ asset('uploads/'.adsCheck($ads20->id)->images) }}" alt="Reklam {{$ads20->id}}" class="img-fluid lazy" data-type="{{ adsCheck($ads20->id)->type }}" height="{{ adsCheck($ads20->id)->height }}" width="{{ adsCheck($ads20->id)->width }}">
                            </a>
                        </div>
                    @endif
                @endif
